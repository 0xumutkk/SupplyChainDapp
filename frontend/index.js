document.addEventListener('DOMContentLoaded', async () => {
  if (typeof window.ethereum !== 'undefined') {
    try {
      ...
      const contractAddress = '0x87497cb1b4DA9f9ff0945ab612A459bD90e28a20'; // Foundry ile deploy ettiğiniz kontrat adresi
      const abi = [ {
        "type": "function",
        "name": "createProduct",
        "inputs": [
      ...
      document.getElementById('inWarehouseForm').addEventListener('submit', async (event) => {
        ...
        const newLocation = document.getElementById('warehouseLocation').value;

        try {
          console.log('Updating product to In Warehouse with data:', { productId, newLocation });
          const tx = await contract.inWarehouse(productId, newLocation);
          await tx.wait();
          alert('Product is now in the warehouse');
        } catch (error) {
          console.error('Error in updating product state to warehouse:', error);
          alert('Error in updating product state to warehouse: ' + error.message);
        }
      });
      ...
      document.getElementById('deliveredForm').addEventListener('submit', async (event) => {
        ...
        try {
          console.log('Updating product to Delivered with ID:', productId);
          const tx = await contract.delivered(productId);
          await tx.wait();
          alert('Product has been delivered');
        } catch (error) {
          console.error('Error in updating product state to delivered:', error);
          alert('Error in updating product state to delivered: ' + error.message);
        }
      });
